...
export class RtcClient {
  ...
  async consume(producerId: string, peerId: string): Promise<RemoteStream | null> {
  ...
  private async createTransport(direction: "send" | "recv") {
    ...
    transport.on("connect", ({ dtlsParameters }, callback, errback) => {
      this.socket
        ...
        .then((r) => {
          if (isError(r)) errback(new Error(`connect failed: code=${r.code}`));
          else callback();
        })
        .catch(errback);
    });
    ...
    if (direction === "send") {
      transport.on("produce", ({ kind, rtpParameters }, callback, errback) => {
        this.socket
          ...
          .then((r) => {
            if (isError(r)) errback(new Error(`produce failed: code=${r.code}`));
